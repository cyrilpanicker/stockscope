...
import {sum as smoothSum,avg as smoothAverage} from './smoothen';
...
export const diPlus = (candles:Candle[],period:number) => {
    const result:DateValue[] = [];
    const smoothDmPlus = smoothSum(dmPlus(candles).slice(1),period);
    smoothDmPlus.unshift({date:candles[0].date,value:null});
    const smoothTrueRange = smoothSum(trueRange(candles).slice(1),period)
...
export const diMinus = (candles:Candle[],period:number) => {
    const result:DateValue[] = [];
    const smoothDmMinus = smoothSum(dmMinus(candles).slice(1),period);
...
export const dx = (candles:Candle[],period:number) => {
    const result:DateValue[] = [];
    const diPlusData = diPlus(candles,period);
    const diMinusData = diMinus(candles,period);
    for(let i=0;i<candles.length;i++){
        const diPlusValue = diPlusData[i].value;
        const diMinusValue = diMinusData[i].value;
        const resultNode:DateValue = {date:candles[i].date,value:null};
        if(diPlusValue!==null && diMinusValue!==null){
            resultNode.value = Math.abs((diPlusValue-diMinusValue)/(diPlusValue+diMinusValue))*100;
        }
        result.push(resultNode);
    }
    return result;
...
export const adx = (candles:Candle[],period:number) => {
    const dxData = dx(candles,period);
    return dxData.slice(0,period).concat(smoothAverage(dxData.slice(period),period));
};
...
// const diPlusData = diPlus(data,14);
// const diMinusData = diMinus(data,14);
// const dxData = dx(data,14);
// const adxData = adx(data,14);
// data.forEach((datum,index)=>{
//     console.log(datum.date+' , '+diPlusData[index].value+' , '+diMinusData[index].value+' , '+dxData[index].value+' , '+adxData[index].value);
// });